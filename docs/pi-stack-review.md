...
## Часть I. «Взять лучшее от обоих» — улучшение своего инструмента

Запрос: не заменять свой инструмент, а улучшить, взяв сильные стороны обоих. Разумно, потому что мы нашли **взаимную дыру**:

| | pi-memory-md | `manage_project_memory` |
|---|---|---|
| Хранение + git | ✅ | ❌ |
| Авто-доставка per-project | ✅ | ❌ |
| **Обзор ВСЕХ проектов** | ❌ **нет** | ✅ был (`list()`) |
| **Поиск по другим проектам** | ❌ **нет** (`scope=project\|global\|all`) | ❌ не было |
| Бюджет/защита от роста | ❌ **нет потолка** | ⚠️ был, но не на той оси |

То есть инструмент не был «плохой версией pi-memory-md» — у него была **уникальная функция**, которой у расширения нет. Её и надо было усилить, а не выбрасывать.

### Разделение ролей (без split-brain)

- **pi-memory-md** — единственный **писатель** + доставка + git.
- **`manage_project_memory`** — **только чтение**, но теперь не legacy-склад, а **кросс-проектный слой** над настоящей памятью.

Дублирования нет: новые действия читают `~/.pi/memory-md/` и ничего туда не пишут.

### Что добавлено (патч [patch-memory-crossproject.diff](patch-memory-crossproject.diff), +262/−1)

| action | Что делает |
|---|---|
| **`index`** | Обзор ВСЕХ проектов: заметки, `description`, размер индекса, флаг превышения бюджета, счётчик недоставляемых заметок. Замена утраченного `list()`, но на новых данных |
| **`search`** | **Regex-поиск по всем проектам сразу** (то, чего нет в расширении). Возвращает `project`, `path`, `line`, `text`, `delivered` |
| **`budget`** | Наследник прежнего лимита 1500, но на правильной оси: размер индекса на проект против бюджета + предупреждение о заметках **вне `core/`** (тихий ноль доставки) |
| `load`/`list`/`delete` | Legacy-хранилище, не тронуто |
| `save` | Остаётся `SAVE_DISABLED` |

### Проверено выполнением (не чтением)

```
index : 9 проектов, 10 заметок, бюджет 8000
        taskbridge  notes=2  chars=692  ~tok=173  [ok]
search: 'пейджинг' -> 6 совпадений в 10 файлах (по разным проектам)
        'ComfyUI'  -> 10 совпадений
budget: total_index_chars=4377, over_budget={}, undelivered={}, warnings=[]
legacy: list=10 тем, load=OK, save=SAVE_DISABLED
pytest: 247 passed
```

**Точность счёта:** мой расчёт даёт `taskbridge = 692` против **691** у реальной функции расширения (погрешность 1 символ, 99.9 %). Достигнуто тем, что я копирую шаблон `memoryContextItemTpl` дословно, включая разворачивание `tags` в `"a, b"` и добавление общей обёртки. Первая версия давала 453 — я забыл обёртку и не разворачивал tags; поймано сверкой с эталоном.

### Что это меняет по сути

Прежний твой лимит `MAX_MEMORY_TOKENS_LIMIT = 1500` **не выбрасывается** — он переосмыслен. Ты ограничивал размер заметки, потому что контент грузился целиком. Контент больше не инжектится, зато появилась новая неограниченная ось: **число заметок × размер индекса**, которая взимается каждую сессию. `action='budget'` и `MEMORY_MD_INDEX_BUDGET_CHARS` закрывают именно её.

---

## Часть J. `pi-doctor` — защита от рецидива документации

### Зачем (обоснование собственными ошибками)

Класс проблемы воспроизвёлся **трижды за одну сессию**:

| # | Что утверждалось | Что было в реальности |
|---|---|---|
| 1 | `AGENTS.md`: расширение `model-state` работает, есть `/modelstate` | `index.tsback` — расширение **не загружается** |
| 2 | `AGENTS.md`: каталог заметок `<Project>/notes/` | расширение доставляет только `<Project>/core/**` — заметки не инжектятся |
| 3 | `skills/small-context-code-agent` числился скиллом | `SKILL.mdback` — скилла нет |

Первые два я записал **сам**, причём №2 — через 40 минут после того, как диагностировал ровно этот класс. Значит, проверка руками не работает, нужна машинная.

### Инструмент

`~/.pi/agent/scripts/pi-doctor.mjs` — сверяет утверждения с файловой системой. Ненулевой exit = есть FAIL.

| Проверка | Что ловит |
|---|---|
| `extensions` | нет загружаемой точки входа (`.ts` / `pi.extensions`); **ссылка в AGENTS.md на мёртвое расширение** |
| `skills` | папка без `SKILL.md` (в т.ч. `.mdback`) |
| `paths` | **абсолютные Windows-пути из `AGENTS.md`, которых нет** ← ловит дефект №2 |
| `memory` | гейт `<Project>/core`; заметки **вне** `core/`; размер индекса против бюджета 8000 |
| `mcp` | `command` с абсолютным путём, которого нет |
| `packages` | `settings.packages`, не установленные в `node_modules` / `local-packages` |
| `models` | `defaultProvider/defaultModel` вне `enabledModels` |

### Проверено на **намеренно сломанной** конфигурации

Собрал песочницу и засеял 5 дефектов — пойманы все:

| Посеянный дефект | Результат |
|---|---|
| расширение только с `index.tsback` | ✅ FAIL (двумя проверками — FS и ссылкой в AGENTS.md) |
| скилл без `SKILL.md` | ✅ WARN |
| проект памяти с `notes/` без `core/` | ✅ FAIL «доставка отключена» |
| `command: G:/nope/missing.exe` | ✅ FAIL |
| `npm:never-installed` в packages | ✅ FAIL |
| путь `...\NotesProject\notes` в AGENTS.md | ✅ FAIL (плейсхолдер и `/memory-check` — верно проигнорированы) |

**Убраны два класса ложных срабатываний** (без этого инструменту перестают верить):
1. regex захватывал текст через перевод строки → ограничен одной строкой;
2. слэш-команды (`/model`, `/memory-check`, `/modelstate`) читались как пути → unix-паттерн удалён.

### Текущий результат на реальной конфигурации

```
[FAIL] extensions  «model-state» не загружается (index.tsback)
[FAIL] extensions  AGENTS.md ссылается на «model-state», которого нет
[warn] skills      «small-context-code-agent»: нет SKILL.md
[ ok ] paths       11 проверено, все существуют
[ ok ] memory      9 проектов, 11 заметок
[ ok ] mcp         15 MCP-серверов
[ ok ] packages    4 установлено
[ ok ] models      llama.cpp/Qwen3.8-27B-ZB4.00-MIN-v5.1-IQ4_XS

2 FAIL, 1 WARN, 5 OK  (exit 1)
```

**Оба FAIL — настоящие**, ложных нет. Инструмент сразу нашёл то, что я держал в отчёте как «известный долг», и подтвердил, что долг реален.

### Ограничение (честно)

`pi-doctor` ловит **расхождение с файловой системой**, но не смысловое устаревание: например, верный путь к тому, что уже не работает. И `AGENTS.md` он читает как текст — если формулировка двусмысленна, проверка может пропустить дефект (в отличие от ложного FAIL, это не защищено).
