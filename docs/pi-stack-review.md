...
| Инжектируемый индекс | ~0 символов | **~283 символа** |
...
|---|---|---|
| Инжектится в сессию автоматически | **0** символов | **283** символов (индекс Taskbridge) |
| Чтение одной темы | 2413 символов (контент 2193 + **220 накладных**, 9%) | 2317 (файл, без обёртки) |
| Обзор всех проектов | 1395 символов / 10 тем, только имя+дата+размер | **1305** символов / 10 проектов, **с description + tags** |
...
**Ключевой вывод:** обзорный индекс B **дешевле** A (1305 против 1395 символов) и при этом **информативнее**
(несёт `description` + `tags`, тогда как A отдаёт лишь имя темы, дату и размер). A не платит на старте, но и не гарантирует,
что память вообще попадёт в контекст.
