...
иначе cmd/PowerShell пишут в OEM-кодировке и кириллица превращается в мусор). В ответ
уходит только ограниченный хвост (около 16 КиБ); если вывод больше, полный текст
сохраняется артефактом сессии (`cmd-output-<время>.log`, виден в разделе Artifacts) и
ответ несёт `{ truncated: true, outputName, outputBytes }` — так большой вывод не
забивает чат и его можно открыть или скопировать позже.
