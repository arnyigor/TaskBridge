...
> **Статус: документ-история.** Описанный здесь split `host ⇄ gateway` был
> реализован (P-1…P-3) и **удалён**: фронт отвечал 6 группам маршрутов из ~40, и
> каждый пришлось бы описать дважды — в IPC-команде и в фронте. Выбран **вариант B**
> (приложение на loopback + LAN-турникет, §12/§12.1) — он даёт тот же результат без
> второй таблицы маршрутов. Разделы 1–11 ниже оставлены как запись разбора;
> актуальное состояние — **§12.1**.
...
> Статус фазы: **P-1…P-3 реализованы и затем удалены** — см. баннер вверху документа
> и §12.1. Формулировки ниже описывают тот путь; актуальный дизайн — вариант B.
...
Статус на 2026-09-14: P-1…P-3 были сделаны (IPC-транспорт, `AgentHost`, gateway,
acceptance) и затем удалены — см. баннер вверху. Дефолт по-прежнему монолит,
