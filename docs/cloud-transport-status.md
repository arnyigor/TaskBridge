...
* **Экран облака убран из интерфейса**: в шапке нет кнопки ☁, в `web/index.html`
  нет ни кнопки, ни оверлея настроек, из `web/app.js` убраны их обработчики
  (настройки, `POST /api/cloud/test`, QR-паринг и список телефонов), из
  `web/app.css` — стили этих форм. Сервер не тронут: ручки `/api/cloud/*`
  (`config`, `test`, `pair`, `devices`) отвечают как раньше, настройку и паринг
  можно сделать запросом; UI вернётся вместе с облаком.
