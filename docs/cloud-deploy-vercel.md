...
**Рекомендация: Git + автодеплой** (мы всё равно работаем в git). Скрипт умеет
настроить это одной командой:

```powershell
npm run cloud:deploy -- --project taskbridge-cloud --database-env POSTGRES_URL --git
```

`--git` подключает репозиторий к проекту (`vercel git connect`, remote берётся из
`git remote get-url origin`), ставит env-переменные и **не** деплоит: дальше деплой
запускает `git push` в production-ветку. Изменённые env-переменные применяются к
следующему деплою — если пушить нечего, добавьте `--redeploy` (пересоберёт
последний production-деплой).

Что выбрать:

* **Git + автодеплой** — рекомендуется: деплой по push, превью-деплои на ветки/PR,
  история и откат в дашборде;
...
Пуш в этом репозитории не сделан: ветка `feature/cloud-transport` локальная. Чтобы
включить автодеплой, запушьте её и выберите production-ветку в настройках проекта
(или смержьте в `main`).
...
| `--git [repo-url]` | подключить Git-репозиторий и не деплоить (автодеплой по push) |
| `--no-deploy` | только env-переменные, без деплоя |
| `--redeploy` | пересобрать последний production-деплой (применить изменённые env) |
...
### 1.6 Проверка, что секреты не уйдут

Отдельная команда, не требующая Vercel:

```powershell
npm run check:secrets          # входит и в npm run check
npm run check:secrets -- -v    # плюс список всех файлов, которые уедут в деплой
```

Что проверяется:

1. **Реальный набор файлов для `vercel deploy`** (обход каталога с учётом
   `.vercelignore`, а не доверие самому файлу) — в нём не должно быть
   `config.json`, `data/`, `cloud/data/`, `artifacts/`, `.env*`, `*.db`, ключей.
2. **`.vercelignore`** содержит обязательные исключения.
3. **`.gitignore`** покрывает локальные секреты/состояние (`git check-ignore`
   проверяет содержимое каталогов, поэтому `data/*` тоже считается).
4. **Значения секретов из локального `config.json`** не встречаются ни в одном
   отслеживаемом файле (`git grep` по каждому значению).
5. **Литералы вида `tb_user_…`, `tb_machine_…`, `postgres://user:pass@…`** в
   отслеживаемых файлах (шаблоны в документации и тестах не считаются).

Команда ничего не печатает из самих секретов и возвращает ненулевой код при
любой находке. Тот же аудит выполняется автоматически перед деплоем: без
чистого `.vercelignore` CLI-деплой не стартует.

Важно: `vercel deploy` **не читает `.gitignore`**, поэтому для CLI-режима
`.vercelignore` обязателен. В Git-режиме Vercel клонирует репозиторий, и
неотслеживаемые файлы физически не попадают в сборку — но `npm run check:secrets`
всё равно стоит запускать перед пушем.
...
* `npm run check`, `npm test` (188 тестов), `npm run test:cloud`, `npm run stress`;
...
* аудит секретов: `tests/cloud-upload-set.test.mjs` проверяет настоящий набор
  файлов для деплоя, а также запускает `check-secrets` на фикстуре с намеренной
  утечкой (секрет в коммите) — утечка обнаруживается, значение в вывод не попадает;
