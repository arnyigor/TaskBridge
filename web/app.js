...
async function openCloudSettings() {
  ...
  // A key shown a minute ago must not still be on screen the next time.
  $('cloudPairResult').classList.add('hidden');
  $('cloudPairQr').innerHTML = '';
...
// The machine mints the QR: it owns the secret. The phone that scans it stores
// the credential and reaches this API through the relay; the list below is the
// only way back — a device token is a signature the relay cannot take back, so
// revocation is enforced by this machine on every frame.

function deviceRow(device) {
  const row = document.createElement('div');
  row.className = `deviceRow${device.revokedAt ? ' revoked' : ''}`;
  const info = document.createElement('div');
  ...
  const meta = document.createElement('div');
  meta.className = 'meta';
  const seen = device.lastSeenAt ? `был на связи ${new Date(device.lastSeenAt).toLocaleString('ru-RU')}` : 'ещё не подключался';
  meta.textContent = device.revokedAt
    ? `отключён ${new Date(device.revokedAt).toLocaleString('ru-RU')}`
    : `${seen} · подключён ${new Date(device.pairedAt).toLocaleDateString('ru-RU')}`;
  info.append(name, meta);
  const action = document.createElement('button');
  action.type = 'button';
  action.className = 'small';
  action.textContent = device.revokedAt ? 'Забыть' : 'Отключить';
  action.onclick = async () => {
    const question = device.revokedAt
      ? `Убрать «${device.name}» из списка?`
      : `Отключить «${device.name}»? Телефон сразу потеряет доступ к этой машине.`;
    if (!confirm(question)) return;
    action.disabled = true;
    try {
      await api(`/api/cloud/devices/${encodeURIComponent(device.deviceId)}`, device.revokedAt ? { method: 'DELETE' } : { method: 'POST', body: '{}' });
...
async function loadCloudDevices() {
  const host = $('cloudDevices');
  try {
    const { devices, realtime } = await api('/api/cloud/devices');
    host.innerHTML = '';
    if (!devices.length) {
      host.textContent = 'Телефоны не подключены.';
      return;
    }
    for (const device of devices) host.append(deviceRow(device));
    if (!realtime) {
      const note = document.createElement('div');
      note.className = 'hint';
      note.textContent = 'Realtime выключен: телефон подключится, когда в настройках облака включат постоянное соединение.';
      host.append(note);
    }
  } catch (error) {
    host.textContent = `Не удалось получить список: ${error.message}`;
  }
}

$('cloudPairButton').onclick = async () => {
  const button = $('cloudPairButton');
  button.disabled = true;
  try {
    const pairing = await api('/api/cloud/pair', { method: 'POST', body: JSON.stringify({ name: `Телефон ${new Date().toLocaleDateString('ru-RU')}` }) });
    // The encoder is only needed on this screen: loaded when it is opened.
    const { default: qrcode } = await import('./vendor/qrcode.mjs');
    const qr = qrcode(0, 'M');
    qr.addData(pairing.url);
    qr.make();
    $('cloudPairQr').innerHTML = qr.createSvgTag({ cellSize: 6, margin: 2, scalable: true });
    $('cloudPairLink').textContent = pairing.url;
    $('cloudPairResult').classList.remove('hidden');
    setCloudStatus(`Ключ действует до ${new Date(pairing.expiresAt).toLocaleDateString('ru-RU')}. Он показан один раз — отсканируйте сейчас.`);
    await loadCloudDevices();
  } catch (error) {
    setCloudStatus(`Не удалось создать ключ: ${error.message}`, true);
  } finally { button.disabled = false; }
};

$('cloudPairCopy').onclick = async () => {
  const ok = await copyText($('cloudPairLink').textContent || '');
  setCloudStatus(ok ? 'Ссылка скопирована — откройте её на телефоне.' : 'Не удалось скопировать, выделите ссылку вручную.', !ok);
};

$('cloudDevicesRefresh').onclick = loadCloudDevices;
