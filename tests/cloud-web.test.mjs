...
test('the shell loads the cloud config before the app, and caches only files that exist', async () => {
  ...
  const app = html.indexOf('type="module" src="/app.js?v=20260916-25"');
  ...
  assert.match(await read('web/cloud-config.js'), /navigator\.serviceWorker\.register\('\/sw\.js\?v=20260916-25'/, 'the PWA must install the current shell revision from an external script');
